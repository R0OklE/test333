...
if(localStorage.getItem('user4')){
    sudokuBoard = JSON.parse(localStorage.getItem('user4'));
  }
  else{
    sudokuBoard = JSON.parse(localStorage.getItem('sudoku4'));
  }
  ...
  function displaySudoku()
  {
      ...
      const prototype = JSON.parse(localStorage.getItem('sudoku4'));
      ...
      if (!isNaN(digit) && digit >= 1 && digit <= 9) {
        ...
        if (selectedCell) {
          ...
          if(prototype[row][col] == 0){
              ...
              localStorage.setItem('user4', JSON.stringify(sudokuBoard));
...
function hintButtonClickHandler() {
  const solution = JSON.parse(localStorage.getItem('solution4'));
  const selectedCell = document.querySelector('.selected');
  const row = parseInt(selectedCell.dataset.row);
  const col = parseInt(selectedCell.dataset.col);
