...
// 生成数独并在控制台显示
if(localStorage.getItem('user6')){
    sudokuBoard = JSON.parse(localStorage.getItem('user6'));
  }
  else{
    sudokuBoard = JSON.parse(localStorage.getItem('sudoku6'));
  }
  ...
  function displaySudoku()
  {
      ...
      const prototype = JSON.parse(localStorage.getItem('sudoku6'));
      ...
      if (!isNaN(digit) && digit >= 1 && digit <= 9) {
        ...
        if (selectedCell) {
          ...
          if(prototype[row][col] == 0){
              ...
              localStorage.setItem('user6', JSON.stringify(sudokuBoard));
...
function hintButtonClickHandler() {
  const solution = JSON.parse(localStorage.getItem('solution6'));
  const selectedCell = document.querySelector('.selected');
  const row = parseInt(selectedCell.dataset.row);
  const col = parseInt(selectedCell.dataset.col);
