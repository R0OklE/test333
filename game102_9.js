...
if(localStorage.getItem('user8')){
    sudokuBoard = JSON.parse(localStorage.getItem('user8'));
  }
  else{
    sudokuBoard = JSON.parse(localStorage.getItem('sudoku8'));
  }
  ...
  function displaySudoku()
  {
      ...
      const prototype = JSON.parse(localStorage.getItem('sudoku8'));
      ...
      if (!isNaN(digit) && digit >= 1 && digit <= 9) {
        ...
        if (selectedCell) {
          ...
          if(prototype[row][col] == 0){
              ...
              localStorage.setItem('user8', JSON.stringify(sudokuBoard));
...
function hintButtonClickHandler() {
  const solution = JSON.parse(localStorage.getItem('solution8'));
  const selectedCell = document.querySelector('.selected');
  const row = parseInt(selectedCell.dataset.row);
  const col = parseInt(selectedCell.dataset.col);
