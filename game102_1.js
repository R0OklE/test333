...
if(localStorage.getItem('user0')){
  sudokuBoard = JSON.parse(localStorage.getItem('user0'));
}
else{
  sudokuBoard = JSON.parse(localStorage.getItem('sudoku0'));
}
...
function displaySudoku()
{
    ...
    const prototype = JSON.parse(localStorage.getItem('sudoku0'));
    ...
    if (!isNaN(digit) && digit >= 1 && digit <= 9) {
      ...
      if (selectedCell) {
        ...
        if(prototype[row][col] == 0){
            ...
            localStorage.setItem('user0', JSON.stringify(sudokuBoard));
...
function hintButtonClickHandler() {
  const solution = JSON.parse(localStorage.getItem('solution0'));
  const selectedCell = document.querySelector('.selected');
  const row = parseInt(selectedCell.dataset.row);
  const col = parseInt(selectedCell.dataset.col);
