...
if(localStorage.getItem('user1')){
    sudokuBoard = JSON.parse(localStorage.getItem('user1'));
  }
  else{
    sudokuBoard = JSON.parse(localStorage.getItem('sudoku1'));
  }
  ...
  function displaySudoku()
  {
      ...
      const prototype = JSON.parse(localStorage.getItem('sudoku1'));
      ...
      if (!isNaN(digit) && digit >= 1 && digit <= 9) {
        ...
        if (selectedCell) {
          ...
          if(prototype[row][col] == 0){
              ...
              localStorage.setItem('user1', JSON.stringify(sudokuBoard));
...
function hintButtonClickHandler() {
  const solution = JSON.parse(localStorage.getItem('solution1'));
  const selectedCell = document.querySelector('.selected');
  const row = parseInt(selectedCell.dataset.row);
  const col = parseInt(selectedCell.dataset.col);
