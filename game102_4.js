...
if(localStorage.getItem('user3')){
    sudokuBoard = JSON.parse(localStorage.getItem('user3'));
  }
  else{
    sudokuBoard = JSON.parse(localStorage.getItem('sudoku3'));
  }
  ...
  function displaySudoku()
  {
      ...
      const prototype = JSON.parse(localStorage.getItem('sudoku3'));
      ...
      if (!isNaN(digit) && digit >= 1 && digit <= 9) {
        ...
        if (selectedCell) {
          ...
          if(prototype[row][col] == 0){
              ...
              localStorage.setItem('user3', JSON.stringify(sudokuBoard));
...
function hintButtonClickHandler() {
  const solution = JSON.parse(localStorage.getItem('solution3'));
  const selectedCell = document.querySelector('.selected');
  const row = parseInt(selectedCell.dataset.row);
  const col = parseInt(selectedCell.dataset.col);
